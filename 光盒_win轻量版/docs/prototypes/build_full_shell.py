...
:root {
  ...
  /* 磨砂浮层 */
  --lb-glass-bg: rgba(255, 255, 255, 0.82);
  --lb-glass-topbar: rgba(243, 245, 247, 0.62);
  --lb-glass-filter: blur(14px) saturate(120%);
  --lb-shadow-float: 0 4px 16px rgba(0,0,0,.08);
  /* 字号（整体比上一版略大） */
  --lb-font: "Segoe UI", "Microsoft YaHei UI", "MiSans Variable", sans-serif;
  --lb-type-display: 17px;
  --lb-type-body: 15px;
  --lb-type-compact: 13px;
  --lb-type-meta: 12px;
  --lb-weight-body: 380;
  --lb-weight-latin: 350;
  /* 圆角分层（拉开差异） */
...
html.theme-dark {
  ...
  --lb-glass-bg: rgba(38, 38, 38, 0.88);
...
.spec-panel {
  ...
  width: 200px; padding: 10px 12px; border: var(--lb-divider-size) solid var(--lb-border);
  ...
  box-shadow: var(--lb-shadow-float); font-size: var(--lb-type-meta); color: var(--lb-text-secondary); line-height: 1.5;
}
...
/* personal homepage modal */
.profile-modal {
  position: absolute; inset: 0; z-index: 400; display: flex; align-items: center; justify-content: center;
  background: rgba(0,0,0,.32);
}
.profile-dialog {
  width: min(680px, 92%); max-height: 80%; display: grid; grid-template-columns: 180px 1fr;
  border: var(--lb-divider-size) solid var(--lb-border); border-radius: var(--lb-r-dialog);
  background: var(--lb-glass-bg); backdrop-filter: var(--lb-glass-filter);
  -webkit-backdrop-filter: var(--lb-glass-filter); box-shadow: var(--lb-shadow-float); overflow: hidden;
}
.profile-nav { padding: 12px; border-right: var(--lb-divider-size) solid var(--lb-divider); background: rgba(0,0,0,.02); }
.profile-nav button {
  display: flex; align-items: center; gap: 8px; width: 100%; height: 34px; padding: 0 10px;
  border: none; border-radius: var(--lb-r-chrome); background: transparent;
  font-size: var(--lb-type-body); text-align: left; cursor: pointer;
}
.profile-nav button.is-active { background: var(--lb-selected); color: var(--lb-accent); }
.profile-body { padding: 20px 24px; overflow: auto; }
.profile-body h2 { font-size: var(--lb-type-display); margin-bottom: 4px; }
.profile-body > p { font-size: var(--lb-type-compact); color: var(--lb-text-muted); margin-bottom: 16px; }
.profile-card {
  display: flex; align-items: center; gap: 14px; padding: 16px;
  border: var(--lb-divider-size) solid var(--lb-border); border-radius: var(--lb-r-card); margin-bottom: 12px;
}
.profile-balance {
  padding: 16px; border: var(--lb-divider-size) solid var(--lb-border);
  border-radius: var(--lb-r-card); background: rgba(10,132,255,.06);
}
.profile-balance strong { font-size: 28px; color: var(--lb-accent); }
...
/* nodes */
.node {
  position: absolute; border-radius: var(--lb-r-card);
  background: var(--lb-glass-bg); backdrop-filter: var(--lb-glass-filter);
...
.canvas-chrome {
  ...
  border-radius: var(--lb-r-pill); background: var(--lb-glass-bg);
...
.composer-topbar-rail {
  ...
  border: var(--lb-divider-size) solid var(--lb-border); border-bottom: none;
  border-radius: var(--lb-r-composer-top) var(--lb-r-composer-top) 0 0;
  background: var(--lb-glass-topbar); backdrop-filter: var(--lb-glass-filter);
}
.composer-topbar-left, .composer-topbar-right { display: flex; align-items: center; gap: 4px; }
.composer-topbar-rail button {
  height: 28px; padding: 0 11px; border: none; border-radius: var(--lb-r-pill);
  background: transparent; font-size: 12px; color: var(--lb-text-secondary); cursor: pointer;
...
.minimap {
  ...
  border: var(--lb-divider-size) solid var(--lb-border); border-radius: var(--lb-r-card);
  background: var(--lb-glass-bg); backdrop-filter: var(--lb-glass-filter); opacity: .9;
}
.minimap-viewport {
...
HTML_BODY = r"""
<div class="proto-badge">完整壳层样板 v2 · 已按反馈修正</div>
...
<div class="spec-panel">
  <h4>描边 & 分割线参数</h4>
  描边 <code>rgba(0,0,0,.10)</code><br>
  强描边 <code>.16</code><br>
  分割线 <code>rgba(0,0,0,.07)</code><br>
  线宽 <code>1px</code><br>
  网格间距 <code>22px</code><br>
  连线粗细 <code>1.05px</code> 固定<br>
  圆角：6/8/10/12/14/22
</div>
...
<div class="app">
  ...
  <div class="body-row">
    ...
    <div class="stage-wrap">
      <div class="stage">
        <div class="canvas">
          <span class="proto-label" style="top:8px;left:8px">画布直角 · 网格 22px</span>

          <!-- 个人主页弹窗 -->
          <div class="profile-modal">
            <div class="profile-dialog">
              <span class="proto-label" style="top:-18px;left:0">个人主页面板</span>
              <nav class="profile-nav">
                <button class="is-active">👤 用户中心</button>
                <button>📊 用量看板</button>
                <button>🕐 历史记录</button>
                <button>⚙ 设置</button>
              </nav>
              <div class="profile-body">
                <h2>用户中心</h2>
                <p>查看账户与可用积分，按需完成充值。</p>
                <div class="profile-card">
                  <div class="user-avatar" style="width:48px;height:48px;font-size:20px">👤</div>
                  <div><strong style="font-size:var(--lb-type-body)">用户</strong><br><span style="font-size:var(--lb-type-compact);color:var(--lb-text-muted)">光盒个人账户</span></div>
                </div>
                <div class="profile-balance">
                  <div style="font-size:var(--lb-type-compact);color:var(--lb-text-muted)">可用积分</div>
                  <strong>2365</strong> <span style="font-size:var(--lb-type-compact)">积分</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="node" style="left:870px;top:290px;width:180px;height:150px">
          ...
          <div class="zoom-menu float-menu">
    ...
    <aside class="dock">
      <div class="dock-chrome">
        ...
        <div class="dock-title-menu float-menu">
          ...
          <div class="dock-foot">
            <div class="dock-foot-left">
              ...
              <div class="skill-wrap">
                <button class="attach-btn">✦</button>
                <div class="skill-menu float-menu">
