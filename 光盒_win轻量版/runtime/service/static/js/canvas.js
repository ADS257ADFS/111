...
function openSmartCanvasPage(id){
    if(!id) return;
    window.location.href = `/static/smart-canvas.html?id=${encodeURIComponent(id)}&v=2026.08.01.21`;
}
...
function renderNode(node){
    ...
    const title = node.type === 'image' ? 'Image' : node.type === 'prompt' ? 'Prompt' : node.type === 'loop' ? tr('canvas.loopNode') : node.type === 'promptGroup' ? 'Prompts' : node.type === 'group' ? 'Group' : node.type === 'output' ? 'Output' : node.type === 'llm' ? 'LLM' : node.type === 'comfy' ? 'ComfyUI' : node.type === 'ltxDirector' ? tr('canvas.ltxDirector'): node.type === 'video' ? tr('canvas.videoGenerateNode') : tr('canvas.apiGenerate');
    const displayTitle = node.type === 'image' && node.url ? nodeTitleForMedia(node) : title;
    // 失败徽章只在一键运行模式中显示，单节点失败已通过 alert 提示
    const showStatus = ['generator','comfy','ltxDirector','llm','video'].includes(node.type) && node.runStatus
    ...
    const canInput = ['generator','comfy','ltxDirector','output','llm','video'].includes(node.type) || (node.type === 'loop' && (node.imageInput || node.showPrompt));
    const canOutput = ['image','prompt','loop','group','promptGroup','generator','comfy','ltxDirector','llm','video','output'].includes(node.type);
...
function renderLLMNodePane(container, node){
    ...
    container.innerHTML = `
        <div class="llm-pane-label">Input${isReadonly ? ' <span style="font-size:10px;opacity:.5;font-weight:600;text-transform:none;letter-spacing:0">(来自连接)</span>' : ''}</div>
...
function renderGeneratorBody(node){
            ...
            <div class="gen-settings-row api-size-row">
                ...
                <div class="gen-count-row">
                    <div class="gen-stepper">
                        ...
                        <input class="gen-count-input" type="text" inputmode="numeric" pattern="[0-9]*" value="${Math.max(1, Number(node.count || 1) || 1)}">
    ...
    countInput.onblur = e => { e.target.value = String(Math.max(1, Number(node.count || 1) || 1)); };
    wrap.querySelectorAll('[data-step]').forEach(btn => {
        btn.onclick = e => {
            e.stopPropagation();
            const next = Math.max(1, Number(node.count || 1) + Number(btn.dataset.step || 0));
            node.count = next;
...
const CANVAS_GENERATOR_TYPES = ['generator','comfy','ltxDirector','video'];
const CANVAS_IMAGE_OUTPUT_TYPES = ['generator','comfy','ltxDirector'];
const CANVAS_MEDIA_OUTPUT_TYPES = ['generator','comfy','ltxDirector','video'];
...
function generatedImageRefs(node){
    const keepGeneratedMedia = ['ltxDirector','video'].includes(node?.type);
...
async function runGenerator(genId, opts={}){
    ...
    const count = Math.max(1, Number(gen.count || 1) || 1);
...
async function callCanvasLLM(node, message, messages=[], options={}){
    const llmProv = resolveChatProviderId(node.llmProvider || 'comfly');const model = resolveChatModel(node.model, llmProv);
...
function canvasRunTypes(){
    return ['generator','comfy','ltxDirector','llm','video'];
}
...
function mergeGeneratedOutputs(node, outputs, append=false){
    ...
    const clean = (outputs || []).map(item => {
        ...
        const kind = node.type === 'video'
            ? 'video'
            : ['ltxDirector'].includes(node.type) && isVideoUrl(url)
                ? 'video'
