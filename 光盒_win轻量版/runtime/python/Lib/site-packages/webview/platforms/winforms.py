    ...
    class BrowserForm(WinForms.Form):
        def __init__(self, window, cache_dir):
            ...
            if (
                ...
                self.Padding = WinForms.Padding(6, 0, 6, 6)
        ...
        def WndProc(self, message):
            if (
                getattr(self, 'frameless', False)
                and self.pywebview_window.resizable
                and message.Msg == 0x0084
            ):
                work_area = WinForms.Screen.FromHandle(self.Handle).WorkingArea
                fills_work_area = (
                    abs(self.Left - work_area.Left) <= 1
                    and abs(self.Top - work_area.Top) <= 1
                    and abs(self.Width - work_area.Width) <= 1
                    and abs(self.Height - work_area.Height) <= 1
                )
                if self.WindowState != WinForms.FormWindowState.Normal or fills_work_area:
                ...
                point = self.PointToClient(Point(screen_x, screen_y))
                border = max(6, int(7 * self._scale))
                # Keep a very thin native right-resize band so the window can
                # still be dragged from the outer edge, while the dock
                # scrollbar (just inside that band) remains clickable.
                right_band = max(3, int(3 * self._scale))
                left = point.X < border
                right = point.X >= self.ClientSize.Width - right_band
                top = point.Y < border
                bottom = point.Y >= self.ClientSize.Height - border
                hit_test = (
                    13 if top and left else 14 if top and right else
                ...
                if hit_test:
                    message.Result = IntPtr(hit_test)
                    return

                titlebar_height = max(30, int(30 * self._scale))
                window_controls_width = max(150, int(150 * self._scale))
                if (
                    screen_y - self.Top < titlebar_height
                    and screen_x < self.Left + self.Width - window_controls_width
                ):
        ...
        def on_shown(self, *_):
            if not is_cef:
                ...
                if is_chromium:
                    try:
                        base_zoom = float(os.environ.get('LIGHTBOX_UI_ZOOM_FACTOR', '1'))
                        self.browser.set_zoom_factor(max(0.25, base_zoom))
                    except Exception:
                        logger.exception('Failed to apply the fixed WebView content size')
