...
def _native_zoom_script(zoom_factor):
    zoom = max(0.25, float(zoom_factor))
    return f"""
        (() => {{
            const zoom = {zoom:.8f};
            const apply = target => {{
                try {{
                    ...
                    root.classList.add('lightbox-native-ui-zoom');
                    root.style.setProperty('--lightbox-native-grid-size', `${{24 / zoom}}px`);
                    root.style.setProperty('--lightbox-native-grid-dot', `${{1 / zoom}}px`);
                }} catch (_) {{}}
            }};
            try {{
                if (window === window.top) window.__lightboxNativeZoomFactor = zoom;
                apply(window);
                if (window === window.top) {{
                    const walk = target => {{
                        apply(target);
                        for (let i = 0; i < target.frames.length; i += 1) walk(target.frames[i]);
                    }};
                    walk(window);
                }}
            }} catch (_) {{ apply(window); }}
        }})();
    """
...
class EdgeChrome:
    ...
    def set_zoom_factor(self, zoom_factor):
        zoom = max(0.25, float(zoom_factor))
        self.webview.ZoomFactor = zoom
        if self.webview.CoreWebView2:
            self.webview.ExecuteScriptAsync(_native_zoom_script(zoom))
    ...
    def on_webview_ready(self, sender, args):
        ...
        initial_zoom = float(os.environ.get('LIGHTBOX_UI_ZOOM_FACTOR', '1'))
        self.webview.ZoomFactor = initial_zoom
        if os.environ.get('LIGHTBOX_UI_ZOOM_FACTOR'):
            sender.CoreWebView2.AddScriptToExecuteOnDocumentCreatedAsync(
                _native_zoom_script(initial_zoom)
            )
    ...
    def on_navigation_start(self, sender, args):
        if self.pywebview_window.transparent and not self.pywebview_window.hidden:
            self.form.Show()
            self.form.Activate()
